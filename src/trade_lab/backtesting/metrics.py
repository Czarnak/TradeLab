...
def _direction_stats(
    trade_log: pd.DataFrame, direction: str
...
def compute_metrics(
    ...
    daily_returns = (
        equity_curve.pct_change().replace([np.inf, -np.inf], np.nan).dropna()
    )
    ...
    if not np.isfinite(annual_vol):
        annual_vol = 0.0
    sharpe = (
        (annualized_return - risk_free_rate) / annual_vol if annual_vol > 0 else 0.0
    )
    ...
    if not np.isfinite(downside_vol):
        downside_vol = 0.0
    sortino = (
        (annualized_return - risk_free_rate) / downside_vol if downside_vol > 0 else 0.0
    )
